...
########################################################  
def MotifsByMotifs(probs, dnas, k):
    mots = []

    for dna in dnas:
       mot = mostProb(dna, probs, k)    
       mots.append(mot)
...
########################################################  
def getRandomMotifs(dnaMotifs):
    mots = []

    for dnamot in dnaMotifs:
       mot = random.choice(dnamot)    
       mots.append(mot)
...
#print(dnaMotifs)    
bestMotifs = getRandomMotifs(dnaMotifs)
#print(bestMotifs)
#motifs = bestMotifs
bestscore = scoreMotifs(bestMotifs)

for i in range(0,1000):
  motifs = getRandomMotifs(dnaMotifs)

#print(dnaMotifs)
  while True:   
    probs = createProfileWithPseudocounts(motifs)
    motifs = MotifsByMotifs(probs, dna, k)
